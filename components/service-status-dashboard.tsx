...
export default function ServiceStatusDashboard() {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div className={`bg-white shadow-md transition-all duration-300 ease-in-out ${isSidebarExpanded ? 'w-64' : 'w-16'}`}>
        <div className="p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className={`text-2xl font-bold ${isSidebarExpanded ? 'block' : 'hidden'}`}>Dashboard</h2>
            <button
              onClick={() => setIsSidebarExpanded(!isSidebarExpanded)}
              className="p-2 rounded-full bg-gray-200 hover:bg-gray-300 transition-colors duration-200"
            >
              {isSidebarExpanded ? <ChevronFirst size={20} /> : <ChevronLast size={20} />}
            </button>
          </div>
          <nav className="space-y-2">
            <button
              className={`flex items-center space-x-2 w-full p-2 rounded-md ${
                activeView === "your" ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100"
              }`}
              onClick={() => setActiveView("your")}
            >
              <LayoutDashboard size={20} />
              {isSidebarExpanded && <span>Your Services</span>}
            </button>
            <button
              className={`flex items-center space-x-2 w-full p-2 rounded-md ${
                activeView === "all" ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100"
              }`}
              onClick={() => setActiveView("all")}
            >
              <Globe size={20} />
              {isSidebarExpanded && <span>All Services</span>}
            </button>
          </nav>
        </div>
      </div>
